 ...
 *  @date    2000, 2009
 *  @date    @verbatim CVS $Date: 2019/03/22 12:58:49 $ @endverbatim
...
/*double compute_true_dsp(double azimuth, double altitude, double corex, double corey, double tel_x, double tel_y)
{
   double tran[3][3];
   get_shower_trans_matrix(azimuth, altitude, tran);
   double new_x,new_y,new_z;
   new_x = tran[0][0] * corex +
           tran[0][1] * corey;
   new_y = tran[1][0] * corex +
           tran[1][1] * corey;
   new_z = tran[2][0] * corex +
           tran[2][1] * corey;
   
   double new_telx, new_tely, new_telz;
   new_telx = tran[0][0] * tel_x+
              tran[0][1] * tel_y;
   new_tely = tran[1][0] * tel_x+
              tran[1][1] * tel_y;
   new_telz = tran[2][0] * tel_x+
              tran[2][1] * tel_y;

   
   //compute the new core in the detector level and the new tel position in detector level
   double x,y;
   double x_tel, y_tel;
   x = new_x - tran[2][0] * new_z/tran[2][2];
   y = new_y - tran[2][1] * new_z/tran[2][2];

   x_tel = new_telx - tran[2][0] * new_telz/tran[2][2];
   y_tel = new_tely - tran[2][1] * new_telz/tran[2][2];

   double tan_theta;
   tan_theta = (new_y - new_tely)/(new_x - new_telx);
   double theta = atan(tan_theta);
   return theta;

}*/

double compute_true_dsp(double azimuth, double altitude, double corex, double corey, double tel_x, double tel_y, double tel_az, double tel_al)
{
   double x1,y1,z1;
   double x2,y2,z2;
   double s1 = 1.0, s2 = 2.0;

   x1 = -s1*cos(altitude)*cos(azimuth)+corex;
   x2 = -s2*cos(altitude)*cos(azimuth)+corex;

   y1 = s1*cos(altitude)*sin(azimuth)+corey;
   y2 = s2*cos(altitude)*sin(azimuth)+corey;
   ...
   az1 = -atan((y1 - tel_y)/(x1 - tel_x));
   az2 = -atan((y2 - tel_y)/(x2 - tel_x));

   //get the distance to the tel position
   double dist1,dist2;
   dist1 = square(x1 - tel_x) + square(y1 - tel_y) + square(z1);
   dist2 = square(x2 - tel_x) + square(y2 - tel_y) + square(z2);

   al1 = asin(z1/sqrt(dist1));
   al2 = asin(z2/sqrt(dist2));

   double xoff1, yoff1;
   double xoff2, yoff2;


   angles_to_offset(az1, al1, tel_az, tel_al, 1, &xoff1, &yoff1);
   angles_to_offset(az2, al2, tel_az, tel_al, 1, &xoff2, &yoff2);

   double theta = atan((yoff2 - yoff1)/(xoff2 - xoff1));

   return theta;
}


double compute_axis_diff(double ph1, double ph2)
{
   double x1, x2, y1, y2;
   
   x1 = cos(ph1), y1 = sin(ph1);
   x2 = cos(ph2), y2 = sin(ph2);
